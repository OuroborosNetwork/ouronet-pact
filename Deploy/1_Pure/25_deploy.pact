...
(interface DeployerReadsV14
    @doc "The COMPLETE public read surface of DPL-UR -- 71 functions in one interface. \
        \ \
        \ SUPERSEDES THE V7..V13 CHAIN, WHICH WAS THE REAL DEFECT. Each of those was ADDITIVE \
        \ and declared only what that revision added -- V13 declared exactly ONE function, \
        \ URC_0035_EliteAccountRichList, and its @doc read \"Elite Account rich-list scan\". \
        \ A module then had to `implements` six or seven of them to describe itself, and any \
        \ consumer had to know which revision introduced the function it wanted. That is a \
        \ changelog wearing the clothes of an interface: it records WHEN a function appeared \
        \ instead of WHAT the module offers. \
        \ \
        \ Worse, the repo module had drifted to `implements DeployerReadsV13` ALONE, so on \
        \ paper it promised one function and delivered seventy -- while the live module still \
        \ implemented V7 + V8. Neither description was true. \
        \ \
        \ One interface, every function, one `implements`. A consumer reads this file and \
        \ knows the whole surface; a reader wanting history reads git."
    ...
    (defun URC_ReverseSwapOutputAmount:decimal
        (swpair:string output-id:string promille:decimal)
    )
    ...
    (defun URC_0019_OrtofungibleButton
        (account:string dpof:string selected-nonces:[integer])
    )
    ...
    (defun URC_0026_CollectablesButtons
    ...
    (defun URC_0001_HeaderV3 (account:string)
        (let
            (
                ...
                (ref-SWPI:module{SwapperIssueV4} SWPI)
                ;;
                ;; DERIVED, NOT HARDCODED. These were twelve literal strings carrying the
                ;; block-hash suffix of the transaction that minted them -- `OURO-8Nh-JO8JO4F5`,
                ;; `Auryndex-O136CBn22ncY`. Any redeploy that re-issues the primordials changes
                ;; every suffix, and this object then reads a set of ids that no longer exist.
                ;; That broke the UI dashboard on 2026-09-24. The same module already derived
                ;; them correctly in URC_PrimordialIDs and URC_0032_EliteAccount -- this
                ;; function was the one place that did not.
                ;;
                ;; RBT DIRECTION where an RBT is known: a reward-BEARING token belongs to
                ;; exactly one pool, whereas a reward token may be paid by several, so
                ;; `(at 0 (UR_RewardToken ...))` is only unambiguous when nothing else pays it.
                ;;
                ;; GOLDENSTOA IS DERIVED THROUGH THE POOL, not through DALOS::UR_GoldenStoaID,
                ;; and that is deliberate: UR_GoldenStoaID reads BAR in the REPL fixture (GSTOA
                ;; is never registered there), so the DALOS route leaves this function
                ;; unrunnable in test -- which is exactly how twelve stale ids shipped
                ;; unnoticed. Measured 2026-09-24: SSTOA -> KORIndex -> PSTOA -> H|PSTOA
                ;; resolves under the fixture and on chain alike.
                (AurynID:string (ref-DALOS::UR_AurynID))
                (EAurynID:string (ref-DALOS::UR_EliteAurynID))
                (SstoaID:string (ref-DALOS::UR_SilverStoaID))
                ;;
                (Auryndex:string (at 0 (ref-DPTF::UR_RewardBearingToken AurynID)))
                (EAuryndex:string (at 0 (ref-DPTF::UR_RewardBearingToken EAurynID)))
                (LiquidIndex:string (at 0 (ref-DPTF::UR_RewardBearingToken SstoaID)))
                (KoriIndex:string (at 0 (ref-DPTF::UR_RewardToken SstoaID)))
                ;;
                (PstoaID:string (ref-ATS::UR_ColdRewardBearingToken KoriIndex))
                (HpstoaID:string (ref-DPTF::UR_Hibernation PstoaID))
                ;;
