;; ===========================================================================================
;; OURONET DEPLOY -- ROUND V2, file 4
;; OUiThreeV1 (interface) + O-UI-THREE (module)  --  OuronetUI entity 3: ELITE ACCOUNT
;; ===========================================================================================
;; INDEPENDENT OF PureV2/01-03. O-UI-THREE imports no AppReads interface -- not even
;; OuronetIdsV1, because every id it needs is derivable from DALOS and none is an ATS pair id.
;; Deploy it in any order relative to the others.
;;
;; WHAT IT REPLACES
;;   DPL-UR::URC_0032_EliteAccount  -> URC_01|EliteAccount   (flat, same 63 keys)
;;   DPL-UR::URC_0035_EliteAccountRichList -> URH_02|RichList
;;   Verified against the page: src/hooks/useEliteAccount.ts and useEliteRichList.ts are the
;;   only callers, and they call exactly these two.
;;
;; TWO CLIENT READS, NOT ONE, AND THAT IS FORCED
;;   The rich list runs `(keys DALOS.DALOS|AccountTable)`. Pact evaluates a `try` body in
;;   READ-ONLY mode where `keys` is disallowed, so it cannot join the panel's composer -- and an
;;   untriable member would make the panel abort wholesale instead of degrade. The UI calls them
;;   in parallel; that costs no extra latency and lets the panel render when the list fails.
;;
;; THE RICH LIST IS THE HEAVIEST READ IN THE APP, and the @doc says so rather than leaving it to
;;   be discovered: it walks every Standard account and then INSERTION-SORTS, which is O(n^2)
;;   under a 10,000,000 gas /local ceiling. Measured 77,046 gas over 14 fixture accounts;
;;   mainnet has ~195. It fits comfortably today and will not forever, and the remedy when it
;;   stops is pagination in the caller rather than a bigger ceiling.
;;
;; A RULE IS DUPLICATED HERE ON PURPOSE. UC_MaxSpecialFeeTargets also exists in O-UI-TWELVE.
;;   Calling across would make one read module a deploy dependency of another and destroy the
;;   property this split buys. RDUI-09 asserts the two copies agree on every tier 0..7, so a
;;   divergence fails the gate -- which is what was missing when DPL-UR's two copies drifted and
;;   told every owner below tier 2 that they had 7 special-fee targets instead of 1.
;;
;; SIGNING
;;   Namespace keyset only. Both are FIRST deploys, so no interface-upgrade hazard and no module
;;   governance check. An UPGRADE of O-UI-THREE later WILL check GOV|O_UI_THREE_ADMIN.
;;
;; MEASURED against Stage 1 + Stage 2 with nothing from this round:
;;   deploy 32,541 gas   ·   URC_01|EliteAccount 25,246 gas, six cards ok
;;   URH_02|RichList 77,046 gas over 14 accounts, ranked high to low
;; ===========================================================================================
