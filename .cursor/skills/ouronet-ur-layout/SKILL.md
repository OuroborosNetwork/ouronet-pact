---
name: ouronet-ur-layout
description: Order of UR (utility read) functions in Ouronet sovereign Pact modules — schema groups, field order, multi-table dispatch, and variable table polling (tbl + with-default-read). Use when adding or rearranging UR_* in core modules (e.g. ANK, AQP, DPDC).
---
...
1. **Groups** — One UR block per **`defschema`** (same order as schemas / tables in the file). See `OuronetInformational/MODULE_ARCHITECTURE.md` § UR helpers.
...
- `OuronetInformational/MODULE_ARCHITECTURE.md` — full prefix rules and greenfield workflow.
