(ouronet-ns.TS01-C1.C_DALOS|RotateGovernor
    ...
    (ouronet-ns.U|G.UEV_GuardOfAny
        [
            (create-capability-guard (ouronet-ns.ATS.ATS|GOV))
            (ouronet-ns.ATS.P|UR "ATSU|RemoteAtsGov")
            (ouronet-ns.ATS.P|UR "TFT|RemoteAtsGov")
            (ouronet-ns.ATS.P|UR "VST|RemoteAtsGov")
        ]
    )
)

(ouronet-ns.DPL-UR.URC_0002_Primordials
    ...
    [
        ...
        "Ѻ.ИMeéłiкνêóœηĄČćlâячNÅνcЭξζû€ŞtÁηΓëgčGěт₿шΣPwȘбуöθłÌñÒИ7жΩЪУqäëš¥3iśÕÏÁζÒъБýξφÇŘÆzû5ÉQAÿκъфЭxΛèõ5țΞVğŚωźЗl∇ÕΓþпνJGφđżμ9Ăтjó$ŮïîkΨŘ7ťμèîVYŒȚÆéÑßëÇÒÚmыИïΨr$ДцńgóЛφ"
        "Ѻ.A0ěьπΨтÎșπЦĐđŽ6ЫêÀεÅĐȘдÞЩ4Ł2ďй5žömiτsλÚÇдěÒaV₱ÏûιЩД₳îJÍşыyÜŹżęìvAЙsÄ¢ÿnΦIťQůЮ7ĄвaèďíoáнõÎLJθÆEáПiXÿÒÀĘ14цU1çΞêSťüIψчèι₱ê9ŽчΓüЦrÀÓμĆ99κQťqPÖшŮ1ÈČSĐŁÌбÝàŞbPσŃĎ8ĄW"
        ...
        "Ѻ.χÃЮTÚиЩąЭÚiWЙhíIgöмEσȘwËøИΦšЙbцÏÑûťt¥ΠпŘbωШþkČĄdѺAПŮćŚUѺÛŠ$₿Ä7bŸфřy¢χíD$IůзbΔğÚÝđğĞłПA7Ìπsžþ¢ÞŻGżóhuÝтѺςлÜЩфuφĘΣЬÝIvĞPjЬçпćœþ6ÃЭođÈηÛЧZÔxìĄZÑĆзLчĎÍhщÇΦăΓ2ÝĞщ₳qл"
    ]
)
...
(module INFO-ZERO GOV
    ...
    ;;{C2}
    ;;{C3}
    (defun CT_KdaPrec ()                    (let ((ref-U|CT:module{OuronetConstantsV2} U|CT)) (ref-U|CT::CT_KDA_PRECISION)))
    ...
    (defun OI|UDC_ClientInfo:object{OuronetInfoV2.ClientInfo}
        (a:[string] b:[string] c:object{OuronetInfoV2.ClientIgnisCosts} d:object{OuronetInfoV2.ClientKadenaCosts} e:list)
        {"pre-text"         : a
        ,"post-text"        : b
        ,"ignis"            : c
        ,"kadena"           : d
    ...
    (defun OI|UDC_FullKadenaCosts:object{OuronetInfoV2.ClientKadenaCosts} (kfp:decimal)
        (let
            (
                ...
                (kadena-text:string
                    (format "Operation costs {} STOA valued at {} with no further discounts applied." [kfp kadena-price])
                )
            )
            ...
            (
                ...
                (kadena-text:string
                    (if (= kadena-discount 1.0)
                        (format "Operation costs {} STOA valued at {} with no further discounts applied." [kadena-need kadena-need-price])
    ...
    (defun DALOS-INFO|URC_ControlSmartAccount:object{OuronetInfoV2.ClientInfo} (patron:string account:string)
        (let
            ...
            (OI|UDC_ClientInfo
                ...
                (if (not is-ignis) (OI|UDC_IgnisCosts patron ifp) (OI|UDC_NoIgnisCosts))
                (OI|UDC_NoKadenaCosts)
                []
            )
        )
