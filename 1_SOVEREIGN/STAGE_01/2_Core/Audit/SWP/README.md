...
| **II — Fixes** | `ROUND-02-FIXES.md` | One entry per fix, applied **sequentially** (owner green-lights each). Living — Fix #1 (C2), Fix #2 (C1), Fix #3 (C3), Fix #4 (C7), Fix #5 (C8), Fix #6 (C9), Fix #7 (H12), Fix #8 (H11), Fix #9 (H6), Fix #10 (C6+H4+H2 combined), Fix #11 (C4, closes H5 too), Fix #12 (H3, SWPT graph redesign) landed. |
...
| H3 | HIGH | SWPT | Principal removal permanently orphans Tracer entries, no resync path | CONFIRMED | **FIXED ✅ AND PROVEN ✅ (`ROUND-02-FIXES.md` Fix #12)** — full redesign, not a patch: `SwapTracerV1`→`V2`, principal-keyed `SWPT|Tracer` replaced with plain token-adjacency `SWPT|Graph`; nothing in SWPT is keyed by principal identity anymore, so orphaning is structurally impossible regardless of remove/replace. Migration utility `SWPI::A_RebuildGraph` added + adversarially proven idempotent. Every prior #13C/#19H/#20H/#11C proof re-run byte-identical post-redesign. Owner's separate "replace-only, cap 7" policy layer not yet built (no longer fix-critical, still wanted for discipline). |
