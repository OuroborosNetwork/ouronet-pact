...
| **I — Owner feedback** | `ROUND-01-OWNER-FEEDBACK.md` | Owner's verdict per finding + new StoicSyntax rules. Living — C10, C13, C12, C5, H9, M8 refuted; C11, H10, H7, H8, M10, M6, M11, M12 design; C2, C1, C7, C8, C9, H12, H11, H6, C6, H4, H2, C4, H5, H3, H1, M9, M13, M7, C3(stable) fixed, C3(weighted) accepted limitation; rest still pending. |
| **II — Fixes** | `ROUND-02-FIXES.md` | One entry per fix, applied **sequentially** (owner green-lights each). Living — Fix #1 (C2), Fix #2 (C1), Fix #3 (C3), Fix #4 (C7), Fix #5 (C8), Fix #6 (C9), Fix #7 (H12), Fix #8 (H11), Fix #9 (H6), Fix #10 (C6+H4+H2 combined), Fix #11 (C4, closes H5 too), Fix #12 (H3, SWPT graph redesign), Fix #13 (H3 follow-up, principal cap + rotate), Fix #14 (H3 2nd follow-up, removal floor + split rotate-self check), Fix #15 (H1, iteration count bumped to 12), Fix #16 (M9, slippage max commented out), Fix #17 (M13, SmartSwap post-swap refresh set uses actual traversed edges), Fix #18 (M7, owner-only gate added to Enable Frozen/Sleeping LP) landed. |
...
| M1–M5, M14 | MED | various | See `ROUND-01-FINDINGS.md` § MEDIUM | mixed | _pending_ |
