...
| H3 | HIGH | SWPT | Principal removal permanently orphans Tracer entries, no resync path | CONFIRMED | **FIXED ✅ AND PROVEN ✅ (`ROUND-02-FIXES.md` Fix #12 + Fix #13)** — full redesign, not a patch: `SwapTracerV1`→`V2`, principal-keyed `SWPT|Tracer` replaced with plain token-adjacency `SWPT|Graph`; nothing in SWPT is keyed by principal identity anymore, so orphaning is structurally impossible regardless of remove/replace. Migration utility `SWPI::A_RebuildGraph` added + adversarially proven idempotent. Every prior #13C/#19H/#20H/#11C proof re-run byte-identical post-redesign. Follow-up policy layer landed too: principals capped at 7, standalone removal permanently disabled, new `SWP::A_RotatePrincipal` (atomic, count-preserving) is the only way to retire one — end-to-end proven a real pool's routing survives rotation byte-identical. |
