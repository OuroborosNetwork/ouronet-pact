...
| **II — Fixes** | `ROUND-02-FIXES.md` | One entry per fix, applied **sequentially** (owner green-lights each). Living — Fix #1 (C2), Fix #2 (C3), Fix #3 (C5), Fix #4 (H1/H2), Fix #5 (H4), Fix #6 (M1), Fix #7 (M2), Fix #8 (M6), Fix #9 (M7), Fix #10 (L1), Fix #11 (L3), Fix #12 (N1) landed. |
...
| L4 | LOW | ATS/ATSU | Hot-RBT surface + ~12 config `C_*` functions have zero REPL coverage (correlates with C5/M1/H4 shipping unnoticed) | CONFIRMED | **ONGOING** — owner confirmed real testing-effort item; confirmed `[6.6]_ATS.repl`, though disabled, does NOT already contain this coverage (checked directly, zero hits). Tracked as a dedicated testing task. |
