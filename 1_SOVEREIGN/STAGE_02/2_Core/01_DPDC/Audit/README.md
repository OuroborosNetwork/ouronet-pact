...
|----|-----|--------|-------|--------|
| C1 | CRIT | DPDC-C | Unsigned `amount` on Credit/Debit → mint supply from nothing | **FIXED ✅ AND PROVEN ✅ (`ROUND-02-FIXES.md` Fix #1)** — live-reproduced pre-fix, `DPDC-T::UEV_AmountsForTransfer` + `DPDC-C::CreditOrDebitDPDC` now floor `amount`, post-fix exploit rejected + legit transfer still works + Z.repl green |
