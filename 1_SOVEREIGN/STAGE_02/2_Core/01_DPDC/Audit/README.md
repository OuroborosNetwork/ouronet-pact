...
| H11 | HIGH | DPDC-UDC/DPDC-S | `UR_N|Score` fails to clamp unscored sentinel in 3/4 branches | **FIXED ✅ AND PROVEN ✅ (`ROUND-02-FIXES.md` Fix #17)** — sentinel check centralized once at function entry, closing all 3 broken branches at once; 8-check live proof (NFT+SFT, unscored+real-score, native+fragment), `git stash` confirms the exact pre-fix bug shape, Z.repl green |
