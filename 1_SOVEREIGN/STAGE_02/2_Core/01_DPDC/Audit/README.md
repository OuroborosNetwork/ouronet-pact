...
| C8 | CRIT | DPDC-S/DPDC-C | `how-many-sets` unbounded (entry-point gap; terminal impact unconfirmed) | **ALREADY CLOSED BY FIX #1 — LIVE-VERIFIED 2026-08-21** — negative rejected at the same `UEV_Amount` chokepoint (full stack trace); zero confirmed harmless (real balances unchanged, only empty rows created). No code change. **All 8 CRITICALs now resolved.** |
