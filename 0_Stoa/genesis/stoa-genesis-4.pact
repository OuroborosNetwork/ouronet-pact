...
(module coin GOVERNANCE
        ...
        (with-capability (URV|COLLECT account)
            (let
                (
                    ...
                    (trial (try false (UR_Balance account)))
                    (iz-account:bool (if (= (typeof (try false (UR_Balance account))) "bool") false true))
                )
    ...
    ;)
    (defun URC_URV|ClaimableRewards:decimal (account:string)
        @doc "Computes Claimable Reward of Account. When only one staker has unclaimed rewards (unclaimed-count=1), that staker receives the full vault supply to avoid rounding dust."
        (let 
            (
