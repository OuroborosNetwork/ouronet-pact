...
(module kadena-coin GOVERNANCE

  @doc "'kadena-coin' is the Kadena mainnet-style native token module used only in this \
  \REPL sandbox. StoaChain native token is the separate root module `coin` loaded from \
  \00_StoaSandbox. This module provides buy/redeem gas support via 'fund-tx', transfer, \
  \credit, debit, coinbase, account creation and query, and SPV burn create. Use fully \
  \qualified `kadena-coin` or `(use kadena-coin)` inside other modules."
  ...
  ;; Blessings omitted: this sandbox module is named `kadena-coin` (not mainnet `coin`).
