...
### Fungible-v2 and kadena-coin
The Kadena-style native token is installed as module **`kadena-coin`** (v6 sources in `kadena/coin-v6.pact`) so the root module name **`coin`** remains free for StoaChain in `00_StoaSandbox`.
