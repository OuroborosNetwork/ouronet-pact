...
| **`XI` / `XB`** | **Persisted writes** under **`require-capability`** on a cap that composes **`SECURE`** or a core cap (e.g. **`SCR|XI>ISSUE-SCORE`**). An **`XI_*` / `XB_*` defun** may use **`UR_*` / `URC_*` / `UC_*` / `UDC_*`** (and **`read`** only through those helpers) to compute what to write, but **must not** **`enforce`** or call **`UEV_*`** — every check belongs in the **`defcap`** (or **`C_`**) that acquires the write capability. **Do not** return **`object{IgnisCollectorV1.OutputCumulator}`**. The **`defun`** body ends at **`insert` / `update` / `write`** (Pact’s native **`string`** return from those ops is incidental — **do not** add **`true`** or other sentinels). **`C_`** builds **IGNIS** cumulators and merges them when several **`X*`** run in one flow. |
| **`XE`** | **Forward-module** entrypoints: **`UEV_IMC`**, then **`with-capability (…\|XE>…)`** **inside** the **`defun`**. The **`defcap`** holds **all** checks local to **this** module + deployed deps (e.g. **`AQP-SCORE`** forward link caps, including **`UEV_*`** used only from that **`defcap`**). The **caller** adds domain rules **before** **`XE_*`**. The **`XE_*` defun body** is **writes (and scoped reads) only**: **no** **`enforce`**, **no domain-validation `UEV_*`** after **`UEV_IMC`** ( **`UEV_IMC`** stays as the inter-module policy hook). The **`defcap`** guard is the single validation surface. **No** **`OutputCumulator`** return — the **forward module’s `C_`** composes IGNIS. |
