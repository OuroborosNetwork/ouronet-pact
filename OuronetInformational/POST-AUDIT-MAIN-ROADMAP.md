...
> | 1.4 AQP module splits | ✅ **CLOSED 2026-09-18 by owner ruling** | The splits were done before this audit began; `04_FVT` → `RPS` + `FVT` was the last. Owner: *"we already did that before we started this, to get to the final Pact code shape."* Measurements agree: nothing exceeds **22%** of a block, `RPS` is **18%**, and the line bands mis-order the population (`MODULE-SIZING.md`, DEFECT-LEDGER §8.29/§8.31) |
