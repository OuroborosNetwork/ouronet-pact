...
## Status: **CLOSED — implemented, measured, proven (2026-08-28); Phase 8 addendum shipped same day, tracked as `#65fL`; Phase 9 shipped same day, tracked as `#65hL`**
...
for the precedent this deliberately mirrors. Seven phases were originally scoped; 6 shipped, 1 was
investigated, measured, and deliberately NOT shipped after the real data contradicted the design
intent. Two more phases followed off-cycle the same day (`#65fL` Phase 8, `#65hL` Phase 9). Real,
measured cumulative result on the established P2-scale benchmark (`SWP|TX 032z2`, `CC_SmartSwap`,
~102 active pools), cold (zero cache) worst case: **5,094,054 → 1,296,898 gas, a 74.5% reduction**
...
77.6% reduction** from the original baseline (unaffected by Phase 7/9 — the bundle-based flow never
calls the repricing loop's own self-searching code path, and a cache hit bypasses BFS entirely
before Phase 9's fix ever runs).

## Final outcome — all 9 phases
...
| **8** (tracked as `#65fL`, `#65bL`/Fix #42 was already closed) | Owner asked about direct-pool-swap costs (confirmed already zero pathfinding) and proposed DLK/OURO get zero-search worth shortcuts — DLK already did (index-based, "liquid staking backwards"); OURO didn't, always fell through to the full graph search. Also found, unprompted, that `URC_HopperActiveShortest` (Liquid Boost's own DLK search) was the one Hopper variant Phases 1-7 never touched — no cache check at all | **Shipped (both sub-phases).** 8a: wired `URC_HopperActiveShortest` to `SWPT\|PathCache`, identical Phase 1 pattern — isolated forced-miss-vs-hit: 276,994 → **26,783 gas (90.3%)**. 8b: new `URC_SingleOuroWorthDWK` (off the primordial pool's own reserves) wired as a new `id==OURO` shortcut in `URC_WorthDWK`/`FromRaw`/`FromGraph`. Caught and fixed a real static recursive-cycle compile error and a real pre-bootstrap crash (OURO branch reachable via `UEV_Issue` before any primordial pool exists) before shipping. Caught a real ~928 gas regression on `SWP\|TX 032q`/`032z2` via `git stash` bisection (a naive extra `UR_OuroborosID` read tripling independent reads of the same DALOS row) — fixed with new `DALOS::UR_CanonicalStoaIds` (DWK/DLK/OURO in ONE read). Net effect on the tracked checkpoints (neither shortcut exercised there): **-878/-895 gas**. Isolated 8b win: 110,099 → **2,452 gas (97.8%)** — but flagged explicitly: the shortcut (spot-ratio) and graph-search (simulated-swap) values genuinely differ (~38% apart at a 100-unit test amount), a real values decision left open for owner review, not hidden as equivalent. **8-addendum:** proven live on a real major-anchored pool (`P\|OURO-98c486052a51\|W1-98c486052a51`, `SWP\|TX 032z8b`): `URC_PoolValue` 10,708 → **3,524 gas (67.1%)**. **8-second-addendum:** measured the minor-principal case directly instead of assuming it (`SWP\|TX 032z8c`, new fixture `MPTEST`, 1 hop from OURO) — major 3,524 gas vs. realistic-minor (1 hop) 120,641 gas vs. worst-case-minor (8 hops, `W7`) 188,205 gas. This overturned a tentative hypothesis that a registration-time principal-proximity policy would help (1 hop is only 1.56x cheaper than 8 hops) — **recommendation: don't pursue that policy**, confirming `UC_BFS` cost scales with graph size scanned, not path depth (the exact gap Phase 9 then closed). |
| **9** (tracked as `#65hL`) | Owner asked, after Phase 8's minor-principal spectrum, whether the search algorithm itself could be made cheaper "if something is closer," or whether something better than BFS exists. Confirmed BFS is already the optimal algorithm CLASS for this unweighted problem (no edge weights/heuristic for Dijkstra/A* to exploit) — but the IMPLEMENTATION never exploited BFS's own early-termination property: `U\|BFS::UC_BFS` computed shortest chains to every reachable node before the caller post-filtered down to the one target needed, a gap the function's own pre-existing `@doc` already admitted | **Shipped.** New `U\|BFS::UC_BFSTargeted(graph, in, target)` — stops once `target` is first visited (BFS visits nodes in non-decreasing distance order, so a node's shortest chain is fixed the first time it's seen; stopping never changes the target's own recorded chain). Additive: `UC_BFS` itself untouched. New `SWPT::URCX_ShortestChainToTarget`/`FromRaw`/`FromGraph` wired into `URC_ComputeGraphPath`/`FromRaw`/`FromGraph`; `URC_ShortestChainPerNode*` untouched. Blast radius checked first: exactly 3 callers of `UC_BFS`, all local to `14_SWPT.pact`. Adversarially proven via sentinel revert-and-compare (gas reverted UP, values stayed byte-identical, then restored) and a new permanent proof (`SWP\|TX 032z8d`) comparing live `UC_BFS` vs `UC_BFSTargeted` on the real ~102-pool topology (52,508 → 50,051 gas, chains byte-identical). Measured: `SWP\|TX 032q` 930,230 → **878,202 gas**; `SWP\|TX 032z2` 1,686,661 → **1,296,898 gas (23.1% further — the largest single win of the whole arc)**; MPTEST (1-hop) 120,641 → **66,926 gas (44.5%)**; W7 (8-hop) 188,205 → **124,358 gas (33.9%)** — closer targets now genuinely cost proportionally less, directly answering the owner's question and confirming their own instinct that nothing beats BFS here (the win was in the implementation, not the algorithm). First regression run after implementing was clean (`EXIT:0, 0 FAILURE`) on the first try. |
...
- `14_SWPT.pact`: `SWPT|TopologyVersion` table + `TopologyVersionRow` schema; `topology-version` field
  ...
  `...FromRaw` pair, sourced from an already-built `[GraphNode]` graph instead. Phase 9:
  `URCX_ShortestChainToTarget`; `URCX_ShortestChainToTargetFromRaw`;
  `URCX_ShortestChainToTargetFromGraph` — early-exit siblings of the `URC_ShortestChainPerNode*`
  trio, now what `URC_ComputeGraphPath*` actually calls (`URC_ShortestChainPerNode*` themselves
  left completely unchanged, still available for any caller wanting the full reachable set).
- `13_U_BFS.pact` / `01_Utilities.pact` (Phase 9): new `UC_BFSTargeted(graph, in, target)` —
  `UC_BFS`'s exact `fold` structure, plus an early-exit no-op once `target` is already in the
  accumulator's `visited` list. `UC_BFS` itself completely untouched. Declared on
  `BreadthFirstSearchV1` alongside the existing `UC_BFS` stub.
...
- `REPL/Stage_01/[6.3]_SWP.repl`: `SWP|TX 032z6b` (Phase 1 warm-cache proof), `SWP|TX 032z6c`
  ...
  032z8a` (Phase 8a warm-cache proof), `SWP|TX 032z8b` (Phase 8 addendum, major-anchored `URC_PoolValue`
  proof on a real pool), `SWP|TX 032z8c` (Phase 8 second addendum, major/realistic-minor/worst-case-minor
  3-point repricing spectrum), `SWP|TX 032z8d` (Phase 9, live `UC_BFS` vs `UC_BFSTargeted` comparison
  on the real ~102-pool topology), all adversarially proven and kept permanently. `SWP|TX 032g`
