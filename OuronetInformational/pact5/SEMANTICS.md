...
- **Numbers:** `integer` (arbitrary precision) and `decimal` (exact for `+ - * /`) are distinct. **Arithmetic auto-promotes** mixed int/decimal → decimal, **but `int / int` truncates** (integer division). `dec` casts int→decimal; `round`/`floor`/`ceiling` 1-arg → integer, 2-arg → decimal. **Verified empirically (SWP audit, 2026-08-17): `floor`/`ceiling` truncate directionally (toward −∞/+∞), NOT banker's rounding** — `(floor 2.999 0)` → `2.0`, `(ceiling 2.001 0)` → `3.0`; only plain `round` (no explicit direction) does round-half-to-even. Force decimal math with `dec`/a decimal literal.
...
- `enforce`/user-guards run **read-only** (5.3), not sys-only — more reads allowed inside guards, still no writes. **This is a real, deliberate sandbox around a real callback capability, not an absence of callbacks** — `create-user-guard`/`enforce-guard` genuinely lets a module invoke caller-chosen, arbitrary code without knowing in advance what it is (Ouronet's own smart-account `governor` rotation depends on exactly this). Verified empirically (SWP audit H9, 2026-08-17): an isolated `enforce-one [(enforce-guard g1) (enforce-guard g2)]` repro where `g2` wraps a function that attempts a table `insert` — the callback genuinely executes, but the write throws `Error during database operation: Operation disallowed in read-only or sys-only mode` at the write site, and **this is not `try`-catchable** (unlike an ordinary `enforce` failure) — it kills the whole transaction outright, no partial/degraded exploit survives. This is the actual reason Pact doesn't have Solidity-style reentrancy: not that callbacks can't happen, but that callbacks arriving via guard/`enforce` evaluation are read-only-sandboxed at the VM level. A callback arriving some *other* way (not through guard evaluation) would need its own check — don't over-generalize this into "no callback can ever cause a problem in Pact."
...
- **`^` (exponentiation) on `decimal` silently drops to IEEE-754 double precision** — NOT exact/arbitrary-precision like `+ - * /` genuinely are for decimals. Verified empirically (SWP audit, 2026-08-17): `(^ 2549.996147035166093620040554 4.0)` = `42282250700760.0859375` vs. the true value via repeated multiplication `42282250700760.099021...` — a ~0.013 absolute error on one call, and integer exponents (`(^ x 4)`) hit the exact same float64 path as decimal ones (`(^ x 4.0)`), so switching exponent type doesn't help. **Whole-number exponents can be made exact via repeated multiplication instead of `^`** (e.g. `(fold (*) 1.0 (make-list n base))`); genuinely fractional exponents (`x^0.3`) have no such workaround in pure Pact — treat any `^` with a non-trivial decimal result as a precision footgun, especially inside iterative/AMM-style math where errors compound. A parameter/binding literally named `exp` will also fail to load — shadows the native exponential function.
