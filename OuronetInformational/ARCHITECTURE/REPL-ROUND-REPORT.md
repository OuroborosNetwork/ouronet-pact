...
|---|---|---|
| `REPL_SUITE_STATS.md` | every scale and coverage figure, plus all 5,490 per-function rows | **generated**: `cd REPL && python3 tools/_suite_stats.py --gate` |
...
| | |
|---|---:|
| **distinct assertions written** | **5,490** |
| **assertions executed per full gate run** | **21,927** |
...
**Quote 5,490 for "how many tests exist" and 21,927 for "how much ran".** They differ ~4x because
...
> fixture loaded by nearly all 89 entrypoints, so each assertion in it runs ~80 times — into
...
A full gate run is **89 entrypoints totalling 6,386 seconds of work — 1 hour 46 minutes serial.**
...
| | |
|---|---|
| gate | **GREEN** — 89 entrypoints, 21,927 assertions, 0 failures |
...
limit on what §2's 21,927 executed assertions certify: they establish that the system behaves as
