...
| | |
|---|---:|
| **distinct assertions written** | **5,505** |
| **assertions executed per full gate run** | **22,027** |
| &nbsp;&nbsp;positive (`expect`) | 17,976 |
| &nbsp;&nbsp;negative (`expect-failure`) | 4,051 |
| gate entrypoints | **90** |
| `.repl` files reachable from the gate | 306 |
| orphaned asserting files (written but never run) | **0** |

**Quote 5,505 for "how many tests exist" and 22,027 for "how much ran".** They differ ~4x because
...
> fixture loaded by nearly all 90 entrypoints, so each assertion in it runs ~80 times — into
...
A full gate run is **90 entrypoints totalling 6,386 seconds of work — 1 hour 46 minutes serial.**
...
| | |
|---|---|
| gate | **GREEN** — 90 entrypoints, 22,027 assertions, 0 failures |
...
limit on what §2's 22,027 executed assertions certify: they establish that the system behaves as
