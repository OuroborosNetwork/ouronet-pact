...
|---|---|---|
| `REPL_SUITE_STATS.md` | every scale and coverage figure, plus all 5,478 per-function rows | **generated**: `cd REPL && python3 tools/_suite_stats.py --gate` |
...
| | |
|---|---:|
| **distinct assertions written** | **5,478** |
| **assertions executed per full gate run** | **21,840** |
...
**Quote 5,478 for "how many tests exist" and 21,840 for "how much ran".** They differ ~4x because
...
> fixture loaded by nearly all 88 entrypoints, so each assertion in it runs ~80 times — into
...
A full gate run is **88 entrypoints totalling 6,386 seconds of work — 1 hour 46 minutes serial.**
...
| | |
|---|---|
| gate | **GREEN** — 88 entrypoints, 21,840 assertions, 0 failures |
...
limit on what §2's 21,840 executed assertions certify: they establish that the system behaves as
