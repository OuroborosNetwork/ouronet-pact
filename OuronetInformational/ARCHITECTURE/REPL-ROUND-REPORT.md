...
| | |
|---|---:|
| **distinct assertions written** | **5,537** |
| **assertions executed per full gate run** | **22,193** |
| &nbsp;&nbsp;positive (`expect`) | 18,095 |
| &nbsp;&nbsp;negative (`expect-failure`) | 4,098 |
| gate entrypoints | **91** |
| `.repl` files reachable from the gate | 306 |
| orphaned asserting files (written but never run) | **0** |

**Quote 5,537 for "how many tests exist" and 22,193 for "how much ran".** They differ ~4x because
...
| | |
|---|---|
| gate | **GREEN** — 91 entrypoints, 22,193 assertions, 0 failures |
...
limit on what §2's 22,193 executed assertions certify: they establish that the system behaves as
