...
> | **W-07** | `UR_NonFungible` wrong column | **CONFIRMED**, then FIXED, and the whole codebase swept for the class (`REPL/_colproj.py`) — 0 further instances, sweep mutation-tested against the known one. |
...
really charges — P-01/02/03/04, now detected automatically by `REPL/_infostoa.py`, which reports 0
...
| # | site | mechanism | status | pin |
|---|---|---|---|---|
| **X-01** | `01_DALOS.pact` `P\|UEV_IMC`, seeded at `Stage_01/[2.1]_Dalos.repl:220` | The genesis sequence registers the **master keyset itself** as a DALOS inter-module policy: `(DALOS.P\|A_AddIMP (keyset-ref-guard "ouronet-ns.dh_master-keyset"))`. `P\|UEV_IMC` is `UEV_Any` over that list, so **any holder of the master key satisfies the "came from a registered module" gate without being a module at all** — `DALOS::C_RotateStoa` executes directly. Two consequences, neither written down: the exception is invisible at every call site, and **anything the admin does on that path is unbilled**, because IGNIS is collected in Talos and this route never enters Talos. It is probably deliberate — the bootstrap must reach DALOS core ops before Talos exists. **The boundary holds exactly where it matters**: the same key cannot *operate* the collector to bill an arbitrary account (`C_Collect` enforces the payer's own ownership). *Skipping your own toll needs nobody else's signature; charging someone else's does.* | **owner** — if intended it belongs in CLAUDE.md beside the "only supported client path" sentence; if not, the registration is what to remove | `<<CONF-01>>` |
...
| **M-09** | `08_DPDC-S.pact` `UEV_Primordial` | Its length guard reports *"Incompatible Input for `<UEV_Composite>` Validation"* — it **names its twin**. Also a live example of the coverage tool's caveat: identical messages mean one test text-pins both. | open | `<<DPDC-S-G1>>` |
...
Twenty-two distinct blind spots were found and fixed in `REPL/_ignis_price_sheet.py` during the
...
| class | entries | fixed | open | owner | accepted |
|---|---:|---:|---:|---:|---:|
