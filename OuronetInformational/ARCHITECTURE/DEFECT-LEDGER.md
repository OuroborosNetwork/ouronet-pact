...
| **GS-03** | `IGNIS-DETER-WORKSHEET.md` header lines 2–7 | The generated worksheet **publishes the pre-calibration formula** — `ceil(update-fields/2)` and read multipliers `1/1/2/3` — while the code that produced every number beneath it is calibrated (`REPL/tools/_ignis_deter_worksheet.py:27` `IG_UPD_FLDDIV = 4`; `_bucket` → `(1,1)/(2,2)/(3,5)/(5,9)`). Only the `print` statements at lines 264 and 267 were never updated. The document `IGNIS-PRICING.md` §1 designates as *"the supporting working the component costs are derived from"* therefore states a formula that contradicts its own contents. | open |
