# Kaddex source dump — part 1 of 4: `exchange`, `aggregator`, `alchemist`

> **Provenance:** pasted verbatim by the owner from a Kadena mainnet block explorer
> (denascan), observed via the `kaddex.*` namespace on **Kadena mainnet** — this is
> **external reference material from a different chain/project, not Ouronet/StoaChain
> code.** Captured 2026-08-28. Not verified against a live node by this session (no
> node query tooling was used — this is a straight paste-to-file transcription of what
> the owner supplied in chat). Treat as "believed accurate as pasted," re-verify against
> a live node/explorer before relying on any exact byte-for-byte claim.
>
> Companion files: `KADDEX-SOURCE-2.md` (`dao`, `fungible-util`, `gas-guards`,
> `noop-callable`), `KADDEX-SOURCE-3.md` (`kdx`, `skdx`, `tokens`), `KADDEX-SOURCE-4.md`
> (`gas-station`, `liquidity-helper`, `staking`, `wrapper`). Read `KADDEX-COMPARISON-HANDOFF.md`
> first — it's the entry point and says what to do with all of this.
