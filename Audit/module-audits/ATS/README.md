...
AQP audit (`Audit/module-audits/AQP/`) — same cycle, same rigor, same format. Goal: a
...
| H2 | HIGH | ATS | Royalty ceiling (99.9%) applies instantly, no lock/timelock | CONFIRMED | **FIXED ✅ AND PROVEN ✅ (`ROUND-02-FIXES.md` Fix #18)** — lock-gate applied via Fix #4; owner rejected a per-tx delta cap as trivially bypassable (same setter callable repeatedly within one tx) and instead asked for a hard ceiling: royalty now capped at 500.0 promile (50%, was 999.0/99.9%) via a royalty-specific enforce in `ATS\|S>ROYALTY`, layered on top of the shared `UEV_Fee` check (left untouched, shared with DPTF). `-1.0`/`0.0` off-sentinels confirmed unaffected. Proven: 999.0 and 500.0001 rejected, 500.0 accepted, both off-sentinels still work. |
...
| L12 | LOW | ATSU | Several master defcaps place a bare-ref validation call before local `enforce`s (StoicSyntax body-order) | CONFIRMED | **KEPT, ONGOING** — owner confirmed 2026-08-18: leave as-is for now, folded into the planned post-audit module rehaul to the newer StoicSyntax variant (same deferral as `#20L`). Confirmed harmless in the meantime — every statement in a `defcap` body still runs top-to-bottom to first failure, so the wrong order only affects which error message a caller sees when multiple conditions are violated at once, plus a hair of extra gas on a doomed call; no path lets through input that should be rejected. No code change. |
| L13 | LOW | Talos | `ATS\|C_SetHotRecoveryFee` (singular, Talos) vs `C_SetHotRecoveryFees` (plural, core) — cosmetic naming asymmetry | CONFIRMED | **LEFT AS-IS ✅** — owner decided 2026-08-18 after being shown the real scope (new interface version + 2 live citizen-module consumers to update + coordinated redeploy). Not worth it for a cosmetic asymmetry. No code change. |
