...
**Version 3.0** · built 2026-09-23 · commit `1333b3d3+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **121,029** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **139,437** |
| distinct assertions written | **1,604** | **5,262** | **5,947** |
...
> **5,947**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,947** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **139,437** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,947** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,947 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,947** |
| distinct assertions counted from the tree now | **5,947** |
...
> The suite's 25,685 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 25,685 assertions** (20,556 positive, 5,129
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,685 during the round
...
`25,685` or `415`, and the assembler substitutes the number it
...
| assertions executed per full gate run | 25,234 | **25,685** |
| distinct assertions written | 5,903 | **5,947** |
