...
**Version 2.0** · built 2026-09-20 · commit `9e8480c+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **116,643** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **138,675** |
| distinct assertions written | **1,604** | **5,262** | **5,903** |
...
> **5,903**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,903** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **138,675** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,903** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,903 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,903** |
| distinct assertions counted from the tree now | **5,903** |
...
> The suite's 25,234 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 25,234 assertions** (20,215 positive, 5,019
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,234 during the round
...
**What This Block Asserts Now.** the attack, re-run, and refused. The bound the original finding established still holds and is still worth stating -- the same key could never operate the collector against a third party, because `IGNIS::C_Collect` enforces the payer's own ownership. *Skipping your own toll needs nobody else's signature; charging someone else's does.* The reachable damage was unbilled ADMIN activity, never drained users. Now it is neither. ==== RT-B-001 - reach a sovereign admin op without Talos, and pay nothing · 01 · env-sigs (caps) ====
...
> WHY THE BOUNDARY HOLDS, mechanically. `P|UEV_IMC` passes only if the caller holds one of the capability guards registered in DALOS's policy list, and those are `create-capability-guard`s over capabilities that only the owning module's own code can bring into scope. A foreign module cannot acquire another module's capability in Pact, so it cannot satisfy the gate no matter what it signs. Registration itself is closed too: `DALOS::P|A_AddIMP` requires `GOV|DALOS_ADMIN`, so an attacker cannot add their own guard to the list. A WHOLE-SURFACE SCAN BACKS THIS UP rather than leaving it at two samples. Of the 290 `C_`/`CC_` implementations across the sovereign core, 289 carry `P|UEV_IMC`. The single exception is `IGNIS::C_TransferDalosFuel`, and it is safe by DELEGATION rather than by a gate: its body is a `coin::transfer`, and Stoa's coin requires the SENDER's own managed TRANSFER capability. That is attacked separately at <<RT-G-002>> rather than taken on trust. ==== RT-G-001b - the adversary attacks the sovereign core · 01 · env-sigs (caps) ==== ===========

### RT-G-002 · REFUSED

*Runs in* `REPL/RedTeam/[RT-G]_HostileCitizen.repl`

**What was suspected.** `IGNIS::C_TransferDalosFuel` is the ONE sovereign client entrypoint of 290 with no `P|UEV_IMC`. Its body is a bare `coin::transfer sender receiver amount` with the sender taken straight from the argument. If the coin layer can be satisfied, a foreign module can move native STOA out of any account it names -- including the Ouronet gas station, which would stop the chain paying for anything.

**How it was driven.** three escalating attempts from the hostile module, each pushing one layer further: (1) call it cold; (2) SIGN AND INSTALL the coin.TRANSFER capability for an account the attacker does not own, and drain the gas station; (3) the same against an ordinary user's account. Messages checked at every step.

**What happened.** refused three times, and the second refusal is the one that matters -- see below.

> WHY THIS ENTRYPOINT HAS NO GATE, and why that is defensible. It is a PRIMITIVE: every `STOA|C_Collect*` path funnels through it, so gating it with `P|UEV_IMC` would gate the collector against itself. CLAUDE.md classes it with the `STOA|C_Collect*` family for exactly this reason. Its authorisation is DELEGATED one layer down to Stoa's coin contract instead of being restated. THE LAYER THAT ACTUALLY HOLDS, and it is not the one people assume. Attempt (1) fails on `Managed capability not installed`, which sounds like the defence but is not: an attacker simply installs it, and attempt (2) does. What stops (2) is that the gas station's STOA account is a `c:` PRINCIPAL whose guard is `(create-capability-guard (DALOS|NATIVE-AUTOMATIC))`, and `coin.pact:144`'s `(enforce-guard (UR_Guard sender))` therefore demands a CAPABILITY, not a signature. No key can satisfy it; only DALOS's own code bringing that capability into scope can. Capability not acquired: CapabilityGuard {name: ouronet-ns.DALOS.DALOS|NATIVE-AUTOMATIC ...} That is the property worth publishing: the sovereign smart accounts are not protected by secrecy or by a signature that could leak, but by a guard that is unsatisfiable from outside the module. An attacker holding EVERY private key in the system still cannot move the gas station's funds. ==== RT-G-002 - drain the gas station through the one ungated entrypoint · 01 · env-sigs (caps) ====
...
`25,234` or `415`, and the assembler substitutes the number it
