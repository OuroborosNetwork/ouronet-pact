...
**Version 2.0** · built 2026-09-22 · commit `3d161799+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **120,052** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **139,238** |
| distinct assertions written | **1,604** | **5,262** | **5,940** |
...
> **5,940**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,940** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **139,238** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,940** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,940 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,940** |
| distinct assertions counted from the tree now | **5,940** |
...
> The suite's 25,668 executed assertions — measured at build time; this book
...
By the round's end 87 had been observed to refuse somebody and
68 of those refusals were attributed to the gate itself rather than to something
...
At the time of writing, the full gate is **green at 25,668 assertions** (20,556 positive, 5,112
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,668 during the round
...
| …with a test that caused them to refuse | 19 | **87** |
| …attributed at depth 0 (the test targeted *this* gate) | — | **68** |
...
`25,668` or `415`, and the assembler substitutes the number it
...
distinguish *"87 of 167 witnessed"* from *"87 of
