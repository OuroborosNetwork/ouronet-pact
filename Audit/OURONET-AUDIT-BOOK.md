...
| Pact source lines | 96,310 | 114,156 | **116,255** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **207** |
| `.repl` lines | 68,435 | 120,603 | **138,196** |
| distinct assertions written | **1,604** | **5,262** | **5,887** |
...
> **5,887**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,887** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **207** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **138,196** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,887** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,887 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,887** |
| distinct assertions counted from the tree now | **5,887** |
...
> The suite's 25,132 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 25,132 assertions** (20,140 positive, 4,992
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,132 during the round
...
`25,132` or `415`, and the assembler substitutes the number it
