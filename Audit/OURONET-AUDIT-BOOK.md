...
**Version 2.0** · built 2026-09-22 · commit `dcd583d8+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **119,415** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **139,205** |
| distinct assertions written | **1,604** | **5,262** | **5,937** |
...
> **5,937**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,937** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **139,205** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,937** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,937 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,937** |
| distinct assertions counted from the tree now | **5,937** |
...
> The suite's 25,647 executed assertions — measured at build time; this book
...
By the round's end 86 had been observed to refuse somebody and
67 of those refusals were attributed to the gate itself rather than to something
it composes; 87 remain unobserved, and Chapter 19 is about why that is
...
At the time of writing, the full gate is **green at 25,647 assertions** (20,546 positive, 5,101
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,647 during the round
...
| …never observed to refuse anybody | 93 | **87** |
| …with a test that caused them to refuse | 19 | **86** |
| …attributed at depth 0 (the test targeted *this* gate) | — | **67** |
...
`25,647` or `415`, and the assembler substitutes the number it
...
distinguish *"86 of 167 witnessed"* from *"86 of
