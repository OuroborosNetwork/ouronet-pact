...
**Version 3.0** · built 2026-09-24 · commit `38e6298b+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **123,587** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **209** |
| `.repl` lines | 68,435 | 120,603 | **140,350** |
| distinct assertions written | **1,604** | **5,262** | **6,042** |
...
> **6,042**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **93** gate entrypoints, **14** fatal static checks, **6,042** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **209** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **140,350** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **6,042** |
...
<sub>The 5,804 here excludes `archive/`; §1's 6,042 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **6,042** |
| distinct assertions counted from the tree now | **6,042** |
...
> The suite's 26,043 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 26,043 assertions** (20,881 positive, 5,162
...
> **That number is a snapshot and will move.** It rose from 22,939 to 26,043 during the round
...
`26,043` or `415`, and the assembler substitutes the number it
...
| assertions executed per full gate run | 25,234 | **26,043** |
| distinct assertions written | 5,903 | **6,042** |
