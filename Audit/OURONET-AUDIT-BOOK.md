...
**Version 3.0** · built 2026-09-24 · commit `3387fdf0+dirty` · generated by `REPL/tools/_auditbook.py`
...
23. [Part V — The canon sweep](#23-part-v-the-canon-sweep)
24. [The canon, and the four shapes a proof can take](#24-the-canon-and-the-four-shapes-a-proof-can-take)
25. [What the sweep found](#25-what-the-sweep-found)
26. [Method: how the round was run](#26-method-how-the-round-was-run)
27. [Appendix A — Reproducing everything](#27-appendix-a-reproducing-everything)
28. [Appendix B — Verification state](#28-appendix-b-verification-state)
...
(Chapter 28) for the verification state — what is proven, what is bounded, and what is explicitly
...
this programme committed before it learned to detect them. Chapter 27 tells you how to re-run
...
Whichever you are, read Chapter 25 if you have read an earlier edition of this book. The
...
So: where a number in this book looks important, Chapter 27 tells you how to reproduce it. If it
...
- **Part V** (Chapters 23–26) — the patron/executor/executee canon
...
- **Appendices** (Chapters 27–28) — reproduction, and the verification state.
...
- **Coverage is entrypoint coverage, not branch coverage.** The worklists in Chapter 28 count
...
|---|---:|---:|---:|
| Pact source files | 91 | 93 | **97** |
| Pact source lines | 96,310 | 114,156 | **122,426** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **209** |
| `.repl` lines | 68,435 | 120,603 | **140,054** |
| distinct assertions written | **1,604** | **5,262** | **6,009** |
...
> **6,009**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **93** gate entrypoints, **14** fatal static checks, **6,009** distinct assertions, orphaned asserting files **0** |
...
> time rather than typed. Chapter 27 says how.
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **209** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **140,054** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **6,009** |
...
<sub>The 5,804 here excludes `archive/`; §1's 6,009 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **6,009** |
| distinct assertions counted from the tree now | **6,009** |
...
| **1.5.1.3** single comprehensive run, one boot, all Pact code | **met by a different mechanism.** The one-boot runner carries 25% of the distinct assertions; the comprehensive run is the 93-way parallel gate |
...
> The suite's 26,010 executed assertions — measured at build time; this book
...
By the round's end 89 had been observed to refuse somebody and
69 of those refusals were attributed to the gate itself rather than to something
...
At the time of writing, the full gate is **green at 26,010 assertions** (20,850 positive, 5,160
...
> **That number is a snapshot and will move.** It rose from 22,939 to 26,010 during the round
...
| …with a test that caused them to refuse | 19 | **89** |
| …attributed at depth 0 (the test targeted *this* gate) | — | **69** |
...
> first showed **171 live instances**, one member accounting for 57. That is the convention, stated
...
`26,010` or `415`, and the assembler substitutes the number it
...
| assertions executed per full gate run | 25,234 | **26,010** |
| distinct assertions written | 5,903 | **6,009** |
...
|---|---|
| Chapter 24 | The four shapes an entrypoint can have, and how to tell them apart |
| Chapter 25 | What the sweep found — three live defects, two wrong published prices, and an authorisation surface measured entrypoint by entrypoint |
| Chapter 26 | The instruments, and the eighteen lessons the round paid for |
...
Chapter 25 were counted rather than asserted.
...
# 24. The canon, and the four shapes a proof can take
...
# 26. Method: how the round was run
...
# 27. Appendix A — Reproducing everything
...
distinguish *"89 of 167 witnessed"* from *"89 of
...
# 28. Appendix B — Verification state
