...
**Version 1.1** · built 2026-09-20 · commit `a5dc847+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **116,432** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **138,568** |
| distinct assertions written | **1,604** | **5,262** | **5,900** |
...
> **5,900**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,900** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **138,568** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,900** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,900 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,900** |
| distinct assertions counted from the tree now | **5,900** |
...
> The suite's 25,232 executed assertions — measured at build time; this book
...
it composes; 84 remain unobserved, and Chapter 19 is about why that is
...
At the time of writing, the full gate is **green at 25,232 assertions** (20,215 positive, 5,017
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,232 during the round
...
| …never observed to refuse anybody | 93 | **84** |
...
`25,232` or `415`, and the assembler substitutes the number it
