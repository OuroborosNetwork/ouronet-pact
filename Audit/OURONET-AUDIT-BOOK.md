...
**Version 2.0** · built 2026-09-22 · commit `1e7999ba+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **120,556** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **139,409** |
| distinct assertions written | **1,604** | **5,262** | **5,946** |
...
> **5,946**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,946** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **139,409** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,946** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,946 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,946** |
| distinct assertions counted from the tree now | **5,946** |
...
> The suite's 25,684 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 25,684 assertions** (20,556 positive, 5,128
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,684 during the round
...
`25,684` or `415`, and the assembler substitutes the number it
