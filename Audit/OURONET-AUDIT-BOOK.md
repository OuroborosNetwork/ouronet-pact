...
**Version 2.0** · built 2026-09-20 · commit `dae0789+dirty` · generated by `REPL/tools/_auditbook.py`
...
| Pact source lines | 96,310 | 114,156 | **116,998** |
| `.repl` files (excl. `archive/`) | 181 | 209 | **208** |
| `.repl` lines | 68,435 | 120,603 | **138,747** |
| distinct assertions written | **1,604** | **5,262** | **5,904** |
...
> **5,904**, measured when this book was built. The canonical value is whatever `ARCHITECTURE/REPL_SUITE_STATS.md`
...
| **1.5** REPL finalisation | a one-command gate over the whole suite | **92** gate entrypoints, **14** fatal static checks, **5,904** distinct assertions, orphaned asserting files **0** |
...
|---|---:|---:|---:|---:|---:|---:|
| `.repl` files (excl. `archive/`) | 181 | — | — | 209 | — | **208** |
| `.repl` lines (excl. `archive/`) | 68,435 | — | — | 120,603 | — | **138,747** |
| distinct assertions | **1,604** | 1,663 | 2,431 | **5,262** | 5,555 | **5,904** |
...
<sub>The 5,804 here excludes `archive/`; §1's 5,904 is the project's own `_suite_stats.py` rule, which
...
| distinct assertions the stats file publishes now (`fb58cba`, 2026-09-18) | **5,904** |
| distinct assertions counted from the tree now | **5,904** |
...
> The suite's 25,236 executed assertions — measured at build time; this book
...
At the time of writing, the full gate is **green at 25,236 assertions** (20,216 positive, 5,020
...
> **That number is a snapshot and will move.** It rose from 22,939 to 25,236 during the round
...
`25,236` or `415`, and the assembler substitutes the number it
