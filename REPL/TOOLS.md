...
**Read this before writing a new analysis script.** There are 71 here, and duplicates have been
...
| `tools/_archivemode.py` | Reduce an OBSOLETE module to ARCHIVE MODE: keep the data and the ability to READ it. |
...
| `tools/_auditdelta.py` | What the v2 audit must re-verify: the SIGNATURE DELTA, generated from git. |
...
| `tools/_callarity.py` | Check CALL-SITE ARITY across the whole tree, statically. |
...
| `tools/_docstrings.py` | PACT STRING-CONTINUATION LINT -- the 2-second check that two 5-minute gate runs did not do. |
...
| `tools/_executorenforced.py` | EVERY `executor` IS PROVEN -- directly, or by a route its own `@doc` names. |
...
| `tools/_hardcodedids.py` | EVERY HARDCODED ENTITY ID IN A DEPLOYABLE SOURCE -- explained, or the tool fails. |
...
| `tools/_liveinventory.py` | Ask the CHAIN what is deployed, and diff it against this tree. |
| `tools/_modref.py` | _modref.py -- modref member calls vs the interface they are typed to. |
| `tools/_modulecomplete.py` | Is a module's sweep turn ACTUALLY finished? One command, PASS/FAIL per obligation. |
...
| `tools/_patronslots.py` | EVERY PATRON SLOT THAT IS NOT THE WORD `patron` -- explained, or the tool fails. |
| `tools/_policyimp.py` | _policyimp.py -- bring every OuronetPolicyV2 implementor to the canonical IMP guard-chain API. |
