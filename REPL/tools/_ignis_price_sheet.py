...
# Flat tolls held as a module-level defconst rather than a map entry, e.g.
# (defconst PYTHIA|REVOKE-IGNIS-FEE:decimal 1.0). PYTHIA|C_RevokeLink bills exactly this and
# published "?" because the extractor only understood the IG|* maps. Restricted to names that
# say fee/price/cost so it cannot start harvesting unrelated constants.
FEE_CONSTS = {}
for _f in glob.glob('1_SOVEREIGN/**/*.pact', recursive=True):
    for _n, _v in re.findall(
            r'\(defconst\s+([A-Za-z0-9|_-]*(?:FEE|PRICE|COST)[A-Za-z0-9|_-]*)\s*:decimal\s+([0-9.]+)\)',
            open(_f).read(), re.I):
        FEE_CONSTS.setdefault(_n, float(_v))
...
def _charge_at(src, core_fn, entity=None, extra='', _depth=3):
    ...
    if 'UC_StoicTagStoaFee' in txt:
        # Both StoicTag ops price off the same helper but collect through DIFFERENT collectors:
        # C_RegisterStoicTag pays it in STOA (STOA|C_CollectWTEx), C_ReleaseStoicTag pays it in
        # IGNIS (IGNIS::C_Collect). Route by the collector actually used, or the release op reads
        # as a STOA charge it never makes.
        if re.search(r'STOA\|C_Collect', txt):
            st.append(('stoa:per-glyph', 1.0))
        else:
            ig.append(('per-glyph', 1.0))
...
for tf in TALOS:
    ...
    for m in re.finditer(r'^    \(defun\s+([A-Za-z0-9-]+)\|([A-Za-z0-9_|+-]+?)(?::[^\s(]+)?\s*\(', src, flags=re.M):
        ...
        talos_body_by_row[(entity, f'{entity}|{fn}')] = body
...
for entity in sorted(rows):
    ...
    for tfn, cf, role, d, comp, reason, ncall, igl, stl in sorted(
        ...
        if d is None:
        ...
        elif not igl and not stl and EMPTY_OC.search(billing_text(_src(MOD2FILE.get(cf_mod[(entity, tfn)], '/dev/null')) if cf_mod.get((entity, tfn)) else '', cf) or ' '):
            # The core op RETURNS an empty cumulator (UC_EmptyOc / EOC): it charges nothing by
            # construction, even though the Talos wrapper wraps it in C_Collect. AQP-VCT's
            # C_AbortVacate is the case -- collecting an empty cumulator collects nothing.
            nexempt += 1
            ...
            nstoaonly += 1
            _sl = " + ".join(f"{lab} {amt:g}" for lab, amt in stl)
            print(f"| {tfn_s} | {cf_s}{compose} | {role} | **0** | {st_cell} | STOA only | "
                  f"no IGNIS charged — priced in STOA only; STOA: {_sl} |")
        elif not igl:
            nunknown += 1
