...
CHANGELOG = [
    ...
    ("2.0", "2026-09-20", [
        ...
        " this, an entrypoint took a `patron` and the actor was either that same account or a value"
        " derived inside a capability and never named -- so 'who is doing this?' had no answer at"
        " the call site, and several functions answered it WRONGLY. Every edit keeps the existing"
        " derived ownership gate and adds the executor check BESIDE it; the derived check was never"
        " replaced, which is the single edit that would have turned this into an authorisation"
        " hole.",
        "SECURITY: the two DPNF anchor paths (ANK|C>ISSUE-DPNF, -DPNF-SET) enforced NO ownership at"
        " all, while their DPTF sibling ran CAP_TF|Owner and their DPSF sibling ran"
        " CAP_OwnerOrCreator. UEV_AssetAnchorCap caps an asset at 49 anchors, so a stranger could"
        " mint 49 anchors against a collection he did not own, exhaust the cap PERMANENTLY, and own"
        " every boost class created along the way. It was being exercised: closing the gate"
        " immediately failed two fixtures and the BOOT itself, all anchoring collections owned by"
        " KST.LUMY while signing only as the admin.",
        "SECURITY: C_IssueSingleScoreModel and C_CombineTripletScoreModel enforced only"
        " 'the patron account exists' -- nothing asserted the caller owned it. Not exploitable,"
        " because the Talos wrappers do reach a patron ownership enforce and P|UEV_IMC makes Talos"
        " the only reachable path; the hole was one layer of defence deep, not zero. Closed anyway,"
        " and closing it is what now lets a sponsor pay for someone else's model."
        " C_IssueMultipletFamily had the same shape and the same fix.",
        "DEFECT (state-corrupting): C_AdmitAgency passed `patron` as the OPERATOR in four places,"
        " one of them WRITTEN TO STATE as DSA|Agency.operator-konto. A sponsored agency open would"
        " have recorded the sponsor -- or, under the gasless patron, DALOS|SC_NAME -- as the"
        " agency's operator permanently. Every other defect in this release is recoverable by"
        " calling again with the right account; this one wrote the wrong owner into a row.",
        "DEFECT: the boost-class owner added in 1.1 was being filled with the PATRON at all four"
        " inline-creation sites, so under a gasless patron every inline boost class would have been"
        " owned by DALOS|SC_NAME rather than its creator -- silently, since nothing reads it back"
        " at issuance.",
        "DEFECT: six DSA entrypoints ran `(enforce (= patron fvt-owner))`, forcing the GAS PAYER to"
        " BE the FVT owner. No sponsor could pay for a vault owner's operation and the gasless"
        " patron was unusable across that entire surface.",
        "DEFECT: MTX|2|C_Inject hardcoded `XB_FvtInject patron patron`, so the documented spike"
        " fallback for the Stage Two daily emission could only run when the gas payer was also the"
        " token source -- which is precisely the shape it exists to rescue. The minter's own @doc"
        " asserted the fallback was available; that assertion was false.",
        "INSTRUMENT: ARCHITECTURE/AUTH-SURFACE.md records, per entrypoint, every ownership enforce"
        " reachable transitively through its capability chain -- 1,104 entrypoints, 813 enforcing."
        " _authsurface.py --check is FATAL in the gate and fails if any entrypoint's set shrinks."
        " As far as we can tell this is the first time the system's authorisation surface has been"
        " written down. It reported 'none weakened, N strengthened' on every commit in this"
        " release, which is how the closed holes were counted rather than claimed.",
        "The Stage Two daily emission (AA_OuroMinterStageTwo) was measured for the first time:"
        " ~940,000 gas, roughly HALF a 2M block, against Stage One's ~135,000. The figure is a"
        " FLOOR, not a ceiling -- four of its six legs scale with staker count. Pinned as a band"
        " rather than a number, because a regression that silently skips legs makes the emission"
        " CHEAPER and without a lower bound that reads as an improvement.",
    ]),
    ("1.1", "2026-09-19", [
...
def _write_release_card():
    """Emit Audit/AUDIT-RELEASE.md -- the one file a publisher needs, GENERATED with the book.

    WHY THIS EXISTS INSTEAD OF A HANDOFF DOCUMENT. A handoff is written once and describes the
    state on the day it was written; the next version silently contradicts it, and nobody finds
    out until a reader does. The publishing surface is small and entirely derivable -- version,
    date, commit, page count, and what changed -- so it is derived. Bump VERSION, rebuild, and the
    card is already correct; there is nothing to remember and nothing to go stale.
    """
    ver, when, items = CHANGELOG[0]
    pages = ""
    try:
        import subprocess
        out = subprocess.run(["pdfinfo", os.path.join(ROOT, "Audit", "OURONET-AUDIT-BOOK.pdf")],
        ...
        pass
    L = [f"# Ouronet Audit Book — release {ver}", "",
         ...
         f"| version | **{ver}** |",
         f"| released | {when} |",
         f"| built from commit | `{_commit()}` |",
         f"| pages | {pages or 'n/a'} |",
         f"| chapters | {len(CHAPTERS)} |",
         "", f"## What changed in {ver}", ""]
    ...
    open(os.path.join(ROOT, "Audit", "AUDIT-RELEASE.md"), "w", encoding="utf8").write("\n".join(L))
...
def main():
    ...
    # THE CARD IS WRITTEN LAST, and that ordering is the point. It reports a PAGE COUNT, which is
    # only known once build_docx has paginated and converged. Written before that (as it was until
    # 2026-09-20) a run that changes the length publishes the PREVIOUS run's count -- so cutting
    # 2.0 produced a card reading 231 pages for a 232-page book. Self-consistent, freshly
    # generated, and wrong: the same shape as the stale gate-log figure found the same day.
    # Two consecutive runs hid it, because the second read the first's number and agreed.
    _write_release_card()
    return 0
