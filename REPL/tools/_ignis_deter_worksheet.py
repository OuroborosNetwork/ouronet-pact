...
def _bucket_header():
    """GS-03: render the bucket line FROM _bucket, never from retyped numbers.

    The previous header hardcoded "S<=5 -> w x1/r x1 ... XL 25+ -> w x5/r x3" and was left
    behind by the substage-6 read calibration, so the document that explains the arithmetic
    contradicted the arithmetic printed underneath it. Boundaries are discovered by probing,
    so a future re-bucketing moves this line automatically.
    """
    rows, prev, start = [], _bucket(1), 1
    for p in range(2, 80):
        b = _bucket(p)
        if b != prev:
            rows.append((start, p - 1, prev)); start, prev = p, b
    rows.append((start, None, prev))
    names = ["S", "M", "L", "XL", "XXL"]
    out = []
    for i, (lo, hi, (w, r)) in enumerate(rows):
        nm = names[i] if i < len(names) else f"B{i}"
        span = f"{lo}-{hi}" if hi is not None else f"{lo}+"
        out.append(f"{nm} {span} \u2192 w\u00d7{w}/r\u00d7{r}")
    return " \u00b7 ".join(out) + "."
SCHEMA_PTS={}
for f in ALLPACT:
...
# ---- OWNER DECISIONS (2026-09-05 batch, IGNIS-PRICING/DECISION-LOG-DETAILED.md) ----
...
def main():
    ...
    print(f"[ {IG_INS}·wbucket per insert/write + ceil(update-fields/{IG_UPD_FLDDIV}) per update"
          f" + rbucket per read/scan + {IG_X} per xcall ]")
    ...
    print("Generated by REPL/tools/_ignis_deter_worksheet.py — OWNER_DECISIONS in-script = the approved batch")
    print("(IGNIS-PRICING/DECISION-LOG-DETAILED.md) and the substage-3 IGNIS defconst input.\n")
    ...
    print(f"\n{tot} ops · {deterred} with deter>1 (rest default 1 or exempt). Regenerate: python3 REPL/tools/_ignis_deter_worksheet.py > OuronetInformational/IGNIS-PRICING/IGNIS-DETER-WORKSHEET.md")
