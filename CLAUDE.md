...
| `REPL/` | Staged loaders and scenario files. **The REPL analysis scripts live in `REPL/tools/`** (moved there 2026-09-14). CORRECTED 2026-09-15: this said *"All 44 analysis scripts"*, and there are **three** tool directories — `REPL/tools/` (the analysis suite and the gate), `tools/` (StoicSyntax canon: `skeleton_emit`/`canon_check`/`cap_band`, plus a `gate.sh` that is **not** the hard gate), and `OuronetInformational/tools/` (the MODULE-INDEX generator). `_toolpaths.py` scans all three; a sentence asserting they all live in one place is how the other two went unchecked. The `REPL/tools/` index is `REPL/TOOLS.md`, generated by `tools/_toolindex.py`. The gate is `python3 REPL/tools/_gate.py`; it resolves its own paths, so it runs from any directory. |
