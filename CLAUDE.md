...
alone **corrupted code**. Applied and gated on 2026-09-17 — 186 files, +9,051/−99, **five suites
...
across 186 files. **Fixed** by a depth guard in `_subdivide_repl.py`: anchors are filtered to lines beginning at paren
depth zero (strings and `;;` comments discounted), and a nested anchor is DROPPED rather than
relocated. Re-running the experiment: 186 files, **0 broken**, gate green at **25,029 — the same
assertion count as before the tool ran**, which is the evidence that 186 files of insertion changed
no behaviour. See DEFECT-LEDGER §8.34.
...
| `REPL/` | Staged loaders and scenario files. **The REPL analysis scripts live in `REPL/tools/`** (moved there 2026-09-14). CORRECTED 2026-09-15, and AGAIN 2026-09-17: this said *"All 44 analysis scripts"*, then said there were **three** tool directories. There are **four** — `REPL/tools/` (the analysis suite and the gate), `tools/` (StoicSyntax canon: `skeleton_emit`/`canon_check`/`cap_band`, plus a `gate.sh` that is **not** the hard gate), `OuronetInformational/tools/` (the MODULE-INDEX generator), and **`scripts/`** — which held `embed-module-interfaces.py`, a tool that rewrites `.pact` sources and **wrote by DEFAULT** (`dry = "--dry-run" in sys.argv`), the exact inversion of the `--apply` rule three paragraphs below, while being invisible to `_toolpaths.py --check`. Fixed: it now requires `--apply`, and `_toolpaths.py` scans all four **and discovers orphans** — it now reports any directory holding `.py` files that `TOOL_DIRS` does not cover, because the 2026-09-15 correction proves a hardcoded list cannot report its own incompleteness. Note what happened here: the sentence added to warn against under-enumeration under-enumerated. The `REPL/tools/` index is `REPL/TOOLS.md`, generated by `REPL/tools/_toolindex.py` (CORRECTED 2026-09-16 — this said `tools/_toolindex.py`, which is the StoicSyntax-canon directory, not where the generator lives; `_toolpaths.py --check` resolves path literals *inside* tools and cannot see a path written in prose). The gate is `python3 REPL/tools/_gate.py`; it resolves its own paths, so it runs from any directory. |
...
- **Citizen** (`2_CITIZEN/`) — extension modules anyone can write. They call **only** into sovereign public APIs; they do not add capabilities to the core surface, and their own client functions carry no `UEV_IMC` (that sovereign inter-module gate belongs to Talos/core paths). The per-asset **launchpad sales** (Spark/Snakes/Custodians/StoicPay/StoicIco) are citizen modules that wire into the sovereign `DEMIPAD` launchpad rules. CORRECTED 2026-09-17 — this sentence used to continue *"`2_CITIZEN/7_Launchpad/99_TS02-DPAD.pact` is the **sovereign** Talos orchestrator … co-located with the launchpad"*, which **contradicted the layout table thirteen lines above it and named a file that does not exist.** The two Talos modules are: `2_CITIZEN/7_Launchpad/99_TS02-**C**PAD.pact`, the **citizen** launchpad Talos carrying the per-sale user wrappers and the gas-funded path; and `1_SOVEREIGN/STAGE_02/3_Talos/05_TS02-**D**PAD.pact`, the **sovereign** one (`DEMIPAD|*`), which is where it lives and is no longer co-located. One letter separates them, and the wrong one was in the sentence that assigns the sovereign/citizen role.
